...
module.exports = {
  ...
  // output: {     publicPath: '/public/',     filename: '[name].[hash].js' },
  devtool: 'cheap-module-eval-source-map', //不包含列信息，同时 loader 的 sourcemap 也被简化为只包含对应行的
  module: {
    rules: [{
      test: /\.vue$/,
      use: 'vue-loader'
    }, { //vue加载器test: /\.tsx?$/,loader: 'ts-loader', //ts加载器
      options: { transpileOnly: true, appendTsSuffixTo: [/.vue$/] //认识vue文件

      } }, {
      test: /\.(css|less)$/, loader: 'vue-style-loader!less-loader!css-loader' //css加载器
    }]
  },
  plugins: [new VueLoaderPlugin(), // vue-loader插件加载方式
  new HtmlWebpackPlugin({ // 此部分新增加template: './public/index.html', //需要自动注入的模板的文件名称
